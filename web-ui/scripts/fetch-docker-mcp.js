    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        servers.push({
          ...
          source_registry: {
            type: 'docker',
            url: `https://hub.docker.com/r/mcp/${name}`,
            verified_by: 'Docker'
          },
          installation_methods: [
            ...
            {
              type: 'docker',
              command: `docker mcp server enable ${name}`,
              requirements: ['Docker Desktop', 'Docker MCP Toolkit']
            }
          ],
          tags: extractTags(name, description),
          file: `docker-mcp/${name}.md`,
          path: `docker-mcp/${name}`
        });
      }
    }
...
function getMockDockerServers() {
  ...
  return mockServers.map(server => ({
    ...
    sources: {
      docker: `https://hub.docker.com/r/mcp/${server.name}`,
      official: `https://hub.docker.com/r/mcp/${server.name}`
    },
    source_registry: {
      type: 'docker',
      url: `https://hub.docker.com/r/mcp/${server.name}`,
      verified_by: 'Docker'
    },
    installation_methods: [
      {
        type: 'bwc',
        recommended: true,
        command: `bwc add --mcp ${server.name}`,
        requirements: ['Docker Desktop', 'Docker MCP Toolkit', 'BWC CLI']
      },
      {
        type: 'docker',
        command: `docker mcp server enable ${server.name}`,
        requirements: ['Docker Desktop', 'Docker MCP Toolkit']
      }
    ],
    tags: extractTags(server.name, server.description),
    file: `docker-mcp/${server.name}.md`,
    path: `docker-mcp/${server.name}`
  }));
}
